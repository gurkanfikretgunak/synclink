  ...
  return (
    <main className="page-enter min-h-full bg-[#faf9f7]">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            {daily.length ? (
              <div className="mt-3 flex h-8 items-end gap-px" aria-label="Clicks, last 14 days">
                {daily.map((row) => {
                  const max = Math.max(1, ...daily.map((d) => d.clicks));
                  const h = Math.max(2, Math.round((row.clicks / max) * 32));
                  return (
                    <div
                      key={row.date}
                      title={`${row.date} · ${row.clicks}`}
                      className="flex-1 bg-neutral-900/80"
                      style={{ height: h }}
                    />
                  );
                })}
      ...
      <aside className="order-first space-y-4 lg:order-none lg:sticky lg:top-8 lg:self-start">
        ...
        {page.slug ? <ShareQr slug={page.slug} /> : null}
